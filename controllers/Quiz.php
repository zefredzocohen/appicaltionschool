...
//		}
	...
	public function index($limit='0')
	{
		...
		$logged_in=$this->session->userdata('user_level');
                if($logged_in==4){
                    $user = $this->session->userdata('member_name');
                    $this->db->from('enrollment');
                    $this->db->join('posts','posts.post_id=enrollment.post_id');
                    $this->db->where('en_name',$user);
                    $check_schol = $this->db->get()->result_array();
                    $post_ids = array();
                    foreach ($check_schol as $key => $value){
                        $post_ids[] = $value['cat_id'];
                    }
                }
		...
		// fetching quiz list
		$data['result']=$this->quiz_model->quiz_list($limit,  isset($post_ids)?$post_ids:NULL);
...
//                var_dump($data['quiz']);die;
	...
	function attempt($rid){
		...
		$data['options']=$this->quiz_model->get_options($data['quiz']['r_qids']);
                $data['count_question'] = $this->quiz_model->get_number_list_anwser($data['quiz']['r_qids']);
//                var_dump($data['count_question']);
//                die;
		$data['title']=$data['quiz']['quiz_name'];
		$this->load->view('admin/quiz_attempt',$data);

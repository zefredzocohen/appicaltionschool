...
Class Quiz_model extends CI_Model
{
  ...
  function quiz_list($limit,$post_id){
...
*/
	 ...
	 $query=$this->db->query("select * from qbank join category on category.cat_id=qbank.cid join level_quiz on level_quiz.lid=qbank.lid 
 ...
 function get_number_list_anwser($qids ){
     $array_qid = explode(',', $qids);
     foreach ($array_qid as $key){
//         echo '<br/>'.$key;
         $result[]=$this->get_number_anwser($key)[0]['abc'];
     }
     return $result;
 }
 function get_number_anwser($qid){
//     $this->db->select_sum('q_option');
//     $this->db->from('options');
//
//    $this->db->where('qid', $qid);
//    $query = $this->db->get();
//
//    $total_sold = $query->row()->q_option;
//echo 'total: '.$total_sold.'<br/>';
//    if ($total_sold > 0)
//    {
//        return $total_sold;
//    }
     $query=$this->db->query("select count(qid)as abc from options where qid='".$qid."'");
	 return $query->result_array();
...
$noq=$quiz['noq'];	
	...
	$query=$this->db->query("select * from qbank join category on category.cat_id=qbank.cid where qid in ($wqids) ORDER BY FIELD(qid,$wqids)  ");	
	...
	foreach($qcl as $k => $val){
		...
		$i=0;
	$query=$this->db->query("select * from qbank join category on category.cat_id=qbank.cid where qbank.cid='$cid' and lid='$lid' ORDER BY RAND() limit $noq ");	
	$questions=$query->result_array();
	foreach($questions as $qk => $question){
		$qids[]=$question['qid'];
		if(!in_array($question['category_name'],$categories)){
 ...
 function open_result($quid,$uid){
	 $result_open=$this->lang->line('open');
		$query=$this->db->query("select * from result  where result.result_status='$result_open' and result.quid='$quid' and result.uid ='$uid' "); 
